  ...
  private buildMailsCreationAnswer = (payload: SendMailPayload[]) => {
    return payload.map(({ employee, creator, event, answer }) => ({
      ...
      Variables: {
        ...
        link: `${isProduction() ? process.env.FRONT_URL : 'http://localhost:3000'}/answer-show-${answer.id}?email=${employee.email}&token=${answer.token}`,
      },
    }))
  }
          ...
          Messages: [
            {
              ...
              Variables: {
                ...
                link: `${isProduction()
                  ...
                  : 'http://localhost:3000'}/answer-show-${answer.id}?email=${employee.email}&token=${answer.token}`,
              },
            },
          ],
        })
